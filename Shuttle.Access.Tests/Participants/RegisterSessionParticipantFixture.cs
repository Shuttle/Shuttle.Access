...
public class RegisterSessionParticipantFixture
{
    ...
    {
        var message = new RegisterSession(_tenantId, IdentityName);
    ...
    {
        var message = new RegisterSession(_tenantId, IdentityName).UsePassword("some_password");
    ...
    {
        var message = new RegisterSession(_tenantId, IdentityName).UsePassword("some_password").WithKnownApplicationOptions(new() { SessionTokenExchangeUrl = "http://localhost" });
        var authenticationService = new Mock<IAuthenticationService>();
    ...
    public void Should_be_able_to_register_a_session_using_delegation()
    {
        ...
        var message = new RegisterSession(_tenantId, IdentityName).UseDelegation(sessionToken);
        var sessionRepository = new Mock<ISessionRepository>();
        var sessionQuery = new Mock<ISessionQuery>();

        var now = DateTimeOffset.UtcNow;
        var session = new Session(_tenantId, Guid.NewGuid(), sessionTokenHash, Guid.NewGuid(), IdentityName, now, now.AddMinutes(1))
    ...
    public void Should_be_able_to_register_a_session_using_a_token()
    {
        ...
        var sessionToken = Guid.NewGuid();
        var sessionTokenHash = hashingService.Sha256(sessionToken.ToString("D"));
        var message = new RegisterSession(_tenantId, IdentityName).UseAuthenticationToken(sessionToken);
        ...
        sessionRepository.Setup(m => m.FindAsync(sessionTokenHash, CancellationToken.None)).Returns(Task.FromResult(new Session(_tenantId, Guid.NewGuid(), sessionTokenHash, Guid.NewGuid(), IdentityName, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow.AddMinutes(1)))!);
    ...
    {
        var message = new RegisterSession(_tenantId, IdentityName).UseDirect();
    ...
    public void Should_be_able_to_renew_a_session()
    {
        ...
        var now = DateTimeOffset.UtcNow;
        var sessionToken = Guid.NewGuid();
        var session = new Session(_tenantId, Guid.NewGuid(), sessionToken.ToByteArray(), Guid.NewGuid(), IdentityName, now, now.AddMinutes(-5));
